...
class MusicInfo(AudioInfo):
    ...
    wav_cond: tp.Optional[WavCondition] = None
    ...
    def from_dict(cls, dictionary: dict, fields_required: bool = False):
        ...
        post_init_attributes = ['self_wav', 'wav_cond', 'joint_embed']
...
class MusicDataset(InfoAudioDataset):
    ...
    def __getitem__(self, index):
        wav, wav_cond, info = super().__getitem__(index)
        ...
        music_info.wav_cond = WavCondition(
            wav=wav_cond[None], length=torch.tensor([info.n_frames]),
            sample_rate=[info.sample_rate], path=[info.meta.path], seek_time=[info.seek_time])

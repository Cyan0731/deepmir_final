...
class AudioDataset:
    ...
    def __getitem__(self, index: int) -> tp.Union[torch.Tensor, torch.Tensor, tp.Tuple[torch.Tensor, SegmentInfo]]:
        if self.segment_duration is None:
            ...
            for retry in range(self.max_read_retry):
                ...
                try:
                    ...
                    wav_cond, sr = audio_read(file_meta.path.replace("accom", "vocal"), seek_time, self.segment_duration, pad=False)
                    out = convert_audio(out, sr, self.sample_rate, self.channels)
                    wav_cond = convert_audio(wav_cond, sr, self.sample_rate, self.channels)
                    n_frames = out.shape[-1]
                    target_frames = int(self.segment_duration * self.sample_rate)

                    if self.pad:
                        out = F.pad(out, (0, target_frames - n_frames))
                        wav_cond = F.pad(wav_cond, (0, target_frames - n_frames))
                    
                    segment_info = SegmentInfo(file_meta, seek_time, n_frames=n_frames, total_frames=target_frames,
                                               sample_rate=self.sample_rate, channels=out.shape[0])
                    
                    # check silence
                    out_utter = torch.mean((torch.abs(out) > 1e-4).float())
                    cond_utter = torch.mean((torch.abs(wav_cond) > 1e-4).float())
                    if out_utter < 0.7 or cond_utter < 0.7:
                        continue
        ...
        if self.return_info:
            ...
            return out, wav_cond, segment_info
        else:
            return out, wav_cond

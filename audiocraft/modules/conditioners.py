...
import spacy
from transformers import RobertaTokenizer, T5EncoderModel, T5Tokenizer, Wav2Vec2FeatureExtractor, AutoModel  # type: ignore
import torch
...
class ConditioningAttributes:
    ...
    wav_cond: tp.Dict[str, WavCondition] = field(default_factory=dict)
    ...
    @property
    def wav_cond_attributes(self):
        return self.wav_cond.keys()
    ...
    def attributes(self):
        return {
            ...
            "wav_cond": self.wav_cond_attributes,
    ...
    def to_flat_dict(self):
        return {
            ...
            **{f"wav_cond.{k}": v for k, v in self.wav_cond.items()},
...
class MertConditioner(WaveformConditioner):
    """Chroma conditioner based on stems.
    The ChromaStemConditioner uses DEMUCS to first filter out drums and bass, as
    the drums and bass often dominate the chroma leading to the chroma features
    not containing information about the melody.
    ...
    Args:
        output_dim (int): Output dimension for the conditioner.
        sample_rate (int): Sample rate for the chroma extractor.
        embed_dim (int): Number of embed_dim for mert.
        duration (int): duration used during training. This is later used for correct padding
            in case we are using chroma as prefix.
        match_len_on_eval (bool, optional): if True then all chromas are padded to the training
            duration. Defaults to False.
        ...
        **kwargs: Additional parameters for the chroma extractor.
    """
    def __init__(self, output_dim: int, sample_rate: int, embed_dim: int,
                 duration: float, match_len_on_eval: bool = True, eval_wavs: tp.Optional[str] = None,
                 n_eval_wavs: int = 0, device: tp.Union[torch.device, str] = 'cpu', **kwargs):
        super().__init__(dim=embed_dim, output_dim=output_dim, device=device)
        self.autocast = TorchAutocast(enabled=device != 'cpu', device_type=self.device, dtype=torch.float32)
        self.sample_rate = sample_rate
        self.match_len_on_eval = match_len_on_eval
        if match_len_on_eval:
        ...
        self.mert = AutoModel.from_pretrained("m-a-p/MERT-v1-330M", trust_remote_code=True).to("cuda")
        self.mert.eval()
        ...
        self.resampler = T.Resample(self.sample_rate, self.resample_rate)
    ...
    def _load_eval_wavs(self, path: tp.Optional[str], num_samples: int) -> tp.Optional[torch.Tensor]:
        ...
        dataset: AudioDataset = AudioDataset.from_meta(
            path, segment_duration=self.duration, min_audio_duration=self.duration,
            sample_rate=self.sample_rate, return_info=True, channels=1)

        if len(dataset) > 0:
            _, eval_infos = dataset.collater([dataset[i] for i in range(num_samples)]).to(self.device)
            eval_wavs = []
            for eval_wav in eval_infos.wav_cond.wav:
                eval_wavs.append(eval_wav)
            eval_wavs = torch.cat(eval_wavs, dim=0)
            logger.info(f"Using {len(eval_wavs)} evaluation wavs for chroma-stem conditioner")
            return eval_wavs
        else:
            raise ValueError("Could not find evaluation wavs, check lengths of wavs")
    ...
    def _get_embed_len(self) -> int:
        """Get length of chroma during training."""
        dummy_wav = torch.zeros((1, int(self.sample_rate * self.duration)))
        if self.resample_rate != self.sample_rate:
            dummy_wav = self.resampler(dummy_wav)
        dummy_inputs = self.processor(dummy_wav.tolist(), sampling_rate=self.resample_rate, return_tensors="pt")
        dummy_inputs = dummy_inputs.to("cuda")
        dummy_chr = self.mert(**dummy_inputs, output_hidden_states=True) # [B, L, T, C]
        dummy_chr = torch.stack(dummy_chr.hidden_states)
        return dummy_chr.shape[-2]


    @torch.no_grad()
    def _extract_embed(self, wav: torch.Tensor, sample_rate: int) -> torch.Tensor:
        """Extract chroma features from the waveform."""
        B, C, T = wav.shape
        if self.resample_rate != sample_rate:
            wav = self.resampler(wav)
        if T < 3300:
            wav = F.pad(wav, (0, 3300-T), "constant", 0) # make minlen
        with self.autocast:
            inputs = self.processor(wav.squeeze(1).tolist(), sampling_rate=self.resample_rate, return_tensors="pt") # [B, T] -> [B, C, T]
    ...
    def _compute_wav_embedding(self, wav: torch.Tensor, sample_rate: int) -> torch.Tensor:
        """Compute wav embedding, applying stem and chroma extraction."""
        # avoid 0-size tensors when we are working with null conds
        return self._extract_embed(wav, sample_rate)
    ...
    def _get_wav_embedding(self, x: WavCondition) -> torch.Tensor:
        ...
        if sampled_wav is not None:
            embed = self._compute_wav_embedding(sampled_wav, self.sample_rate)
        else:
            assert all(sr == x.sample_rate[0] for sr in x.sample_rate), "All sample rates in batch should be equal."
            embed = self._compute_wav_embedding(x.wav, x.sample_rate[0])

        if self.match_len_on_eval:
            B, T, C = embed.shape
            if T > self.embed_len:
                embed = embed[:, :self.embed_len]
                logger.debug(f"Embed was truncated to match length! ({T} -> {embed.shape[1]})")
            elif T < self.embed_len:
                n_repeat = int(math.ceil(self.embed_len / T))
                embed = embed.repeat(1, n_repeat, 1)
                embed = embed[:, :self.embed_len]
                logger.debug(f"Embed was repeated to match length! ({T} -> {embed.shape[1]})")
...
def dropout_condition(sample: ConditioningAttributes, condition_type: str, condition: str) -> ConditioningAttributes:
    ...
    Works in-place.
    """
    if condition_type not in ['text', 'wav', 'wav_cond', 'joint_embed']:
        raise ValueError(
            "dropout_condition got an unexpected condition type!"
            f" expected 'text', 'wav', 'wav_cond' or 'joint_embed' but got '{condition_type}'"
        )
    ...
    if condition_type == 'wav':
        wav = sample.wav[condition]
        sample.wav[condition] = nullify_wav(wav)
    elif condition_type == 'wav_cond':
        wav_cond = sample.wav_cond[condition]
        sample.wav_cond[condition] = nullify_wav(wav_cond)
...
class ClassifierFreeGuidanceDropout(DropoutModule):
    ...
    def forward(self, samples: tp.List[ConditioningAttributes]) -> tp.List[ConditioningAttributes]:
        ...
        samples = deepcopy(samples)
        for condition_type in ["wav", "wav_cond", "text"]:
            for sample in samples:
...
class ConditioningProvider(nn.Module):
    ...
    def wav_conditions(self):
        return [k for k, v in self.conditioners.items() if isinstance(v, WaveformConditioner) and not isinstance(v, MertConditioner)]
    ...
    @property
    def wav_cond_conditions(self):
        return [k for k, v in self.conditioners.items() if isinstance(v, MertConditioner)]
    
    @property
    def has_wav_cond_condition(self):
        return len(self.wav_cond_conditions) > 0
    ...
    def tokenize(self, inputs: tp.List[ConditioningAttributes]) -> tp.Dict[str, tp.Any]:
        ...
        wavs_cond = self._collate_wavs_cond(inputs)
        joint_embeds = self._collate_joint_embeds(inputs)

        assert set(text.keys() | wavs.keys() | wavs_cond.keys() | joint_embeds.keys()).issubset(set(self.conditioners.keys())), (
            f"Got an unexpected attribute! Expected {self.conditioners.keys()}, ",
            f"got {text.keys(), wavs.keys(), wavs_cond.keys(), joint_embeds.keys()}"
        )

        for attribute, batch in chain(text.items(), wavs.items(), wavs_cond.items(), joint_embeds.items()):
    ...
    def _collate_wavs_cond(self, samples: tp.List[ConditioningAttributes]) -> tp.Dict[str, WavCondition]:
        """Generate a dict where the keys are attributes by which we fetch similar wavs,
        and the values are Tensors of wavs according to said attributes.
        ...
        *Note*: by the time the samples reach this function, each sample should have some waveform
        ...
        for sample in samples:
            for attribute in self.wav_cond_conditions:
                wav, length, sample_rate, path, seek_time = sample.wav_cond[attribute]
                assert wav.dim() == 3, f"Got wav with dim={wav.dim()}, but expected 3 [1, C, T]"
                assert wav.size(0) == 1, f"Got wav [B, C, T] with shape={wav.shape}, but expected B == 1"
                # mono-channel conditioning
                wav = wav.mean(1, keepdim=True)  # [1, 1, T]
